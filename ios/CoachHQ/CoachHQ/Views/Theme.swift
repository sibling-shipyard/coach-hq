...
enum Theme {
    ...
    static let ink = Color(uiColor: UIColor { trait in
        trait.userInterfaceStyle == .dark
            ...
            : WarmInstrumentTokens.Palette.UI.ink
    })
    ...
    static let mutedBackground = Color(uiColor: UIColor { trait in
        trait.userInterfaceStyle == .dark
            ...
            : WarmInstrumentTokens.Palette.UI.desk
    })

    /// Card corner radius — Warm Instrument's `radius.cardIosPt` (18pt).
    static let cornerRadius: CGFloat = WarmInstrumentTokens.cardRadius
...
/// `shared/warm-instrument/tokens.json` → `WarmInstrumentTokens.generated.swift` (codegen).
enum WarmInstrument {
    ...
    static let surfaceMuted = Color(uiColor: UIColor { trait in
        trait.userInterfaceStyle == .dark
            ...
            : WarmInstrumentTokens.Palette.UI.surfaceMuted
    })
    static let ink = Theme.ink
    static let inkMuted = WarmInstrumentTokens.Palette.inkMuted
    static let inkFaint = WarmInstrumentTokens.Palette.inkFaint
    ...
    static let accent = WarmInstrumentTokens.Palette.accent
    static let accentDark = WarmInstrumentTokens.Palette.accentDark

    /// The one "something's wrong" color — cold indigo-grey. Never stack alarms.
    static let alarmBg = WarmInstrumentTokens.Palette.alarmBg
    static let alarmFg = WarmInstrumentTokens.Palette.alarmFg

    static let border = Theme.cardBorder
    static let borderDashed = WarmInstrumentTokens.Lines.borderDashed
    static let headerRule = WarmInstrumentTokens.Lines.headerRule

    static let cardShadow = WarmInstrumentTokens.Shadows.card
    static let engineShadow = WarmInstrumentTokens.Shadows.engine

    /// `radius.cardIosPt` — same value as `Theme.cornerRadius`, named here to match the JSON key.
    static let cardRadius: CGFloat = WarmInstrumentTokens.cardRadius
    ...
    static func color(hex: String) -> Color {
        WarmInstrumentTokens.color(hex: hex)
    }

    /// Static sport palette from codegen — used only as a fallback where a snapshot item
    /// doesn't carry its own hex (most do; commitments/mix always do).
    static let sportColors: [WarmSportId: Color] = WarmInstrumentTokens.sportColors
