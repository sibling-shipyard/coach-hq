...
- [ ] Full-bleed accent background per workout type (Foundation blue, Calisthenics terracotta — not beige)
- [ ] Cascade stagger reveal: sport icon bounces in → time trophy → three stat cards one at a time
- [ ] Three-beat haptic pattern on appear (da-da-DUM) instead of single `.success()`
- [ ] Coach note as a proper serif card (WarmCard + coachVoice font, large) not a bare quote
- [ ] CTA routing: "Talk to Coach →" (primary) + "Back to Home" (secondary) — NOT back to workout list

### Animation & Feel (apply existing system more fully)
- [ ] Home widget column: apply `staggerReveal` to each card (modifier already exists, unused here)
- [ ] `HairlineProgress`: animate from `fraction: 0` on appear — every quest/calorie/progress bar in the app
- [ ] Card press style: add spring scale (0.97) alongside existing opacity dim across all list cards
...
### Chrome Polish
- [ ] Timer back button: `Text("←")` → `Image(systemName: "chevron.left")` with semibold weight
- [ ] Dock start CTA: `Text("▶ Start workout")` → `Image(systemName: "play.fill")` + `Text("Start workout")`
- [ ] WorkoutListView: add page header (title + date, matches CompactInstrumentHeader pattern from Home)
...
- [ ] Background HealthKit delivery: wake the app when a new workout is written to HK — even when closed
- [ ] Auto-trigger sync on HK workout detection (user plays badminton, app knows without them opening it)
- [ ] Push notification: "Badminton session logged — Coach is looking at it"
- [ ] Notification tap: deep link into Activity Detail or Coach Chat for that specific session
...
- [ ] First Coach Chat open: a waiting message — "Here's how I think about your training..."
...
- [ ] Personalized time-of-day greeting ("Good morning, [name]" / "Rest day — recover well")
- [ ] Today's scheduled workout shown in header: tap → WorkoutOverview inline
- [ ] Skeleton shimmer loading: cards matching actual widget layout instead of centered ProgressView
- [ ] All progress bars: animate from 0 on first data load across Engine, Quest, Calories

### Coach Chat Feel
- [ ] Contextual composer placeholder: "How did that feel?" post-workout / "What's on your mind?" default
- [ ] Quick-react chips on new/empty thread: "Felt strong 💪" / "Struggled today" / "Question about plan"
- [ ] Post-workout auto-chips: pre-seeded chip row after a completed workout or HK sync
- [ ] Coach bubble: slightly elevated cream card feel vs. plain text on background

### Workout Tab
- [ ] "Today" hero section at top: today's assigned workout prominent, clear Start button
- [ ] Visual separation: "TODAY'S PLAN" vs "AVAILABLE WORKOUTS" sections

### Navbar Rethink
- [ ] Add labels below tab icons (standard iOS pattern — removes ambiguity for new users)
- [ ] Or: stronger selected state — larger filled icon + brief label on first tap (tooltip style)
