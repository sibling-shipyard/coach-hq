...
- `GET /api/coach-chat?profileStatus=1` (query-param dispatch on the existing route, not a standalone file) for iOS to poll on launch without a chat turn. **Not a separate file**: Vercel's Hobby plan caps a deployment at 12 serverless functions (one per top-level `api/*.ts`), and this repo sits exactly at that cap after Part A merged — a standalone `coach-chat-profile-status.ts` file (the original design) pushed it to 13 and broke the build. Folded into `coach-chat.ts`'s GET handler instead. Worth knowing for any future new endpoint: fold into an existing route, or free up headroom by consolidating `coach-chat-context.ts` (A3) the same way first.
