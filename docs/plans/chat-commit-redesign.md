...
| D1 | D — Reliability | Validation self-corrects; a commit failure never discards Coach's reply; a dropped action is always signaled, not just hoped-for in the next reply | A1 | `coachReplySchema.ts`, `geminiClient.ts`, `coachIntents.ts`, `coachTurn.ts`, web + iOS error handling | [`ccr-d1-validation-lld.md`](ccr-d1-validation-lld.md) | Closes #736, no silent data loss on bad output or GitHub failure |
