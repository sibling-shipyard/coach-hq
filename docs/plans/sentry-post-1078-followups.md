...
| **P1** | Benchmark give-up has no terminal tag | Retries already `captureServerException` (B8); after max attempts the permanent give-up is only `console.error` — triage cannot tell “dead for this athlete” from the three prior failures | **Yes — PR1** (one capture/tag at give-up) |
...
**PR1** first — three soft-null sites + one give-up tag. Smaller than the original #1108 Done-when item 2.
