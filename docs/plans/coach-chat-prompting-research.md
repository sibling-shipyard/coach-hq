...
| 4 | Retry-with-repair for JSON-truncation, instead of blind retry | **Not built.** `coachLlmClient.ts`'s retry is still blind — same request replayed once on a stale-cache 400 or a 503/504, no re-ask-with-the-malformed-output-and-fix-this variant. | `ui/api/coach-chat/_lib/coachLlmClient.ts:92-104` |
