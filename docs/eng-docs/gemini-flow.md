...
| `coaching_style_update`                               | prompt reinforcement + `findMissedCoachingStyleLanguage` (first-session only, while no style is on file)                                                                                          | -                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
The reprompt is one call shared by every violation. What happens when a problem survives it:
...
7. **Training frequency has its own field.** `training_availability_update` (days per week, optional weekdays) writes `memory.training_availability`, so it lands like sports or style do (ADR 0052). The completion turn prefers that value and falls back to parsing the memory notes only when the model never set it.
8. **A reprompt keeps the first reply's intake fields.** The reprompt regenerates the reply from scratch, so `carryOverDroppedIntakeFields` (`turnReplyValidation.ts`) keeps `season_start`, `coaching_style_update`, `sports_update`, `training_availability_update`, `memory_update`, `injury_flag` and missing `profile_update` entries from the first reply when the second omits them. The reprompt wins on a conflict, and an oversized field is never carried. Each carry records a `reprompt_fields_carried` silent fixup for Sentry.
