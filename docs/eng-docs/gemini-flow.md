...
stating a durable training pattern ("I always run better in the evening") together with another
request in the same message ("also be more direct with me") got the other action field
(`coaching_style_update`) but `memory_update` was silently never in the model's JSON - 3 of 4 real
attempts on `coach-date2022`. `unrecorded_facts` came back empty too, so the existing reprompt
never fired, and the reply's own prose still claimed the pattern was noted.
...
and "logged" are generic filler this codebase's own reply text uses constantly for completely
unrelated acknowledgments that have nothing to do with a memory-worthy fact - several existing
fixtures in `coachTurn-reprompt.test.ts` use a bare `reply: "Noted."` alongside an `injury_event`
or `quest_event` write, with no durable pattern in sight. Gating on "another action field also
fired" doesn't rescue it: a non-filler turn almost always has some other action field set, so this
would reprompt on a large share of ordinary compound turns, not just the real drop. That's the
same class of risk the gap-2a generic keyword match and #1072's narration guard were already
rejected for.
