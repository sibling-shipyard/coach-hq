...
| `multi-field-success` | `manual-coach-chat-turns-multi-field-success.json` | 3 (incl. greet) | new (#1105 B2); one message asking for a permanent `template_edit` and a this-week-only `week_update` (swap two days) together, after seeding a real week - proves two real action fields can both land from one turn, not just fail together (`fullTurnPipeline.test.ts` only ever proved the failure case) | turn 1: `current_week.json` and `workout_plans/templates/` both changed; turn 2: PASS |
