...
> Status: Current · Owner: vade-the-tester · Verified: 2026-09-15
...
and against this catalog's own eval/simulation tables above. "Gap → closed" means this pass added
the row that closes it; "Gap → left open" means I found it but chose not to build a fixture this
round, with the reason. `pending_clarification`/`unrecorded_facts` aren't in this table - they're
self-audit fields, not writes (see the schema doc).

| Action field | Writes to | Eval coverage | Simulation coverage | Status |
|---|---|---|---|---|
| `coach_note` | `coach_log.json` | `02`, `13`, `14`, most others | fires on nearly every turn across all scenarios, but no scenario asserts `coach_log.json` itself changed | Covered (eval); simulation checks are implicit only - left open, see note below |
| `memory_update` | `memory.json` | `15` | `pattern-style-sport` (new) | Gap → closed |
| `coaching_style_update` | `memory.json` | `09`, `15` | `pattern-style-sport` (new) | Gap → closed |
| `sports_update` | `memory.json` | `15` | `pattern-style-sport` (new) | Gap → closed |
| `injury_flag` | `injuries.json` | `08`, `13`, `14` | `fsp-basic`, `daily-basic`, `injury-resolve-by-bodypart` (new) | Gap → closed |
| `injury_event` | `injuries.json` | `14` | `injury-resolve-by-bodypart` (new) | Gap → closed |
| `quest_event` | `progress.json` | `16` | `quest-event` (new) | Gap → closed |
| `profile_update` | `profile.json` | `04`, `12`, `20` (new) | `fsp-basic`, `daily-basic` | Covered |
| `season_start` / `.new_habits` | `seasons.json`, `quests.json` | `08`, `10`, `12`, `20` (new) | `season-transition` (new) | Gap → closed |
| `quest_create` (standalone) | `quests.json` | `11` | none | Gap → left open (see note) |
| `template_edit` | template file | `05` (absence only - never asserted present) | none | Gap → left open (see note) |
| `session_plan` | session-snapshot file | `19` (new) | `session-plan` (new) | Gap → closed (was the redesign-followups doc's zero-coverage item) |
| `week_update` (kickoff) | `current_week.json` | `06` | `ambiguous-contradiction`, `week-kickoff-flash` (new) | Gap → closed (Flash malformation retest) |
...
- **`template_edit` (permanent edit) has never been asserted as *present* anywhere**, only as
  *absent* (transcript `05`'s disambiguation). Its write-time logic
  ...
  covers for `session_plan` and `workout_create` (same file, same validation pattern), so the
  residual risk is lower than the other closed gaps. Flagging as a P2 follow-up, not building a
  fixture this round given the marginal coverage it would add.
- **`coach_note`'s own file write has no dedicated simulation assertion.** It fires on nearly every
  turn in every scenario in this library, so it's exercised constantly in practice - the gap is
  narrower than the others (no scenario's `expect` block happens to check `coach_log.json`
  specifically). Not worth a dedicated fixture; any future scenario touching an ordinary turn
