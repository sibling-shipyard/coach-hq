...
Both adapters call `withLlmSpan` (`_lib/sentry.ts`) with `{"llm.adapter": "gemini" | "openrouter"}`,
so a Sentry trace always names which one ran. `captureLlmFailure`'s `model` field is filled from
`adapter.model` at every call site - never a hardcoded model constant, which would silently
mislabel an OpenRouter failure as a direct-Gemini one. A caller like `askLlm` that resolves its
own adapter internally instead tags the model onto the thrown error, and reads it back at the
`captureLlmFailure` call site.

### Usage accumulation across a retry

`span.setAttributes` (the mechanism `withLlmSpan`'s `recordUsage` callback writes through)
...
`LlmUsage` object across attempts, summing each numeric field. Preserve `undefined` when
...
2. **Chat** (`coach-chat/_lib/llm/coachLlmClient.ts`'s `askLlm`) - moved onto the seam in M2
...
- Tag `.model` onto any thrown error if the caller resolves its own adapter and has its own
  `captureLlmFailure` call site downstream, the same way `askLlm` does - the two-line
...
- Every `captureLlmFailure` call site's `model` field traces back to a real adapter, not a
