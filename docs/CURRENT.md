...
| Doc | Role |
|-----|------|
| [`hq-restructure-plan.md`](hq-restructure-plan.md) | **Active** HQ in-repo hygiene — four-band layout (shared, ui, ios, platform, engine) |
| [`hq-port-plan.md`](hq-port-plan.md) | HQ port from clean `coach-phelps` (P1–P3) — **done**; superseded for next moves by `hq-restructure-plan.md` |
