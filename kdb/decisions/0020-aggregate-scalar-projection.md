...
- **Area:** cross-cutting
- **Context:** The `gen/aggregate.json` file produced by `build-aggregate.mjs` and used by the UI loads the full history on boot. Currently, it includes 85+ legacy Strava fields predating ADR 0010. Furthermore, upcoming features like PR 162 will introduce `hr_stream` (9,413 bytes per activity on average). Without intervention, time-series arrays and heavy fields (like `hr_stream`, `segment_efforts`, `map`, `laps`, `splits_metric`, `splits_standard`) make the JSON payload balloon to >7MB.
