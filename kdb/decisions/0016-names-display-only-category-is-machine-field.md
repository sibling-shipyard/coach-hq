...
- **Rejected:** (1) `naming_config.json` per athlete (issue #143 original proposal) — unnecessary once names aren't machine identifiers. (2) Big-bang auto-category in Phase 1 — too many consumers to change at once; the manual field and generic naming ship first.
- **Enforces:** A display string is never a machine identifier. If code regex-matches a name to decide behaviour, the field it wants does not exist yet — add it.
