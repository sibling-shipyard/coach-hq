...
- **Decision:** **One canonical schema — version 4.** All user repos converge on it. Spec: [`docs/challenge-v2-schema.md`](../docs/challenge-v2-schema.md). Required: `season`, `main_quest`, `quests`. Optional: `phase`, `milestones`, `weekly_targets`, `graduated`. **No top-level `challenge` block** — the old 60-day kickstart is a `season` (short or nested under a longer arc via `phase`).
