...
That's the only fake in this file - request-body construction (`coachLlmClient.ts`), the
explicit-cache lookup, the cache-retry-on-400 branch, the timeout-retry branch (all
`geminiAdapter.ts`), and response parsing (`coachLlmClient.ts`) are all real, unmodified code.
...
**Start here:** `coachLlmClient.test.ts`. One test (`issue #609`) documents that `askLlm` does no
