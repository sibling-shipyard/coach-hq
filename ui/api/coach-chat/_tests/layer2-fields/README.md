...
Tests decision -> file content: the pure appliers (`coachIntents.ts`, `coachWeekFiles.ts`,
`coachWorkoutFiles.ts`, `turnWrites/*.ts`) that take the current JSON plus a parsed action and
