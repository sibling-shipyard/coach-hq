...
no git commit happen in this folder — those are `gemini/` (the Gemini boundary) and `commit/`
(the atomic write path) respectively.

"Decide" means: given a parsed `GeminiReply` and the athlete's current files, work out exactly
...
| `turnWrites/`                   | One write-builder per `GeminiReply` action field — see its own [README](turnWrites/README.md) |
