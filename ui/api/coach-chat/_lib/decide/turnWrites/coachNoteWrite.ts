...
// coachProfileIntents.ts's applyCoachNote for the pure merge logic this wraps with I/O.
...
export function buildCoachNoteWrite(
  repo: string,
  token: string,
  // The turn's own cached "today" (turnRequest.ts's loadTurnState), not recomputed here - the
