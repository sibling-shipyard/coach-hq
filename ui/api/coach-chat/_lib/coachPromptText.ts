          ...
          : mode === "closing"
            ...
            : firstSession
              ? [
                  ...
                  "required); season_start as soon as the first season is agreed; and quest_create as",
                  "soon as the main goal or habit quests are agreed. Omit a field when this turn added no",
                  "fact for it.",
              ...
              : [
                  ...
                  "quest_id. Omit a field entirely when this turn added no fact for it - never invent",
                  "one to fill a guess. Do not set coach_note, template_edit, session_plan, week_plan,",
                  "session_reconcile, or plan_edit on an ordinary turn; those remain close-only.",
                  `memory_update.text: max ${MEMORY_NOTE_TEXT_CAP} chars. injury_flag[].text / injury_event[].text: max ${INJURY_FLAG_TEXT_CAP} chars.`,
                  SAVE_CLAIM_GUARD,
                  SESSION_STAYS_OPEN,
                ].join("\n"),
