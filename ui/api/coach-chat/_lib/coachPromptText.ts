  ...
  return [
    ...
    mode === "greeting"
      ...
      ? [
          ...
          "week plan yet.", SAVE_CLAIM_GUARD, "\n" + SESSION_CLOSE_DECISION,
        ].join("\n")
      : mode === "closing"
      ? [
          ...
          "have them).", "Only set this when you are genuinely committing the week now, not while still asking",
...
export function activeWeekSessionsContext(
  ...
  return [
    ...
    "match the date to what the athlete actually means, never guess):", ...lines,
  ].join("\n");
}
