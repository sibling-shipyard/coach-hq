...
| File | Role |
|---|---|
| `coachChatFiles.ts` | Raw repo file reads + `CoachContext` (SOUL/quest_log/profile/memory/injuries/coach_log) loading, with an in-flight/short-TTL cache; profile-complete check |
...
| `coachWrites.ts` | Write-authority: closing-turn file context, `coach_since` stamping (currently thin — see the rebuild note above) |
