  ...
  return (
    <section className="wg-section" id={id}>
      <div className="wg-section__meta">
        <span className="wg-section__kicker">
          {index} · {title}
        </span>
      ...
      <div className="wg-page">
        ...
        <div className="wg-intro">
          ...
          <p className="wg-intro__lede">
            Every widget below is the real production component from Home, fed by fixture data
            instead of the repo's live feed. Hover, drag, and page through them — the interactions
            are the actual ones the athlete gets, not a mockup.
          </p>
        ...
        <GallerySection
          ...
          interaction={
            <>
              Move across the Engine's 6-week trend for per-week load values. Quest and Coach's Read
              have no tap targets in this release — Quest logging and the coach thread live on their
              own pages.
            </>
          }
          asides={[
            {
              ...
              content: (
                <>
                  Home swaps this rail for a compact Quest bar paired with Calories, and drops
                  Coach's Read entirely (the thread continues in Coach Chat). Shown full-size here
                  for reference at every width.
                </>
              ),
            },
          ]}
        ...
        <GallerySection
          ...
          interaction={
            <>
              Tap the badminton record line to toggle ALL ⇄ RANKED — the only tap target on Home in
              this release. The calisthenics cube below is showing its alarm state: floor missed,
              "The bar is cold."
            </>
          }
        >
        ...
        <GallerySection
          ...
          interaction={
            <>
              Drag a sport chip onto another day — empty days accept, occupied days swap, and the
              projection recomputes live and turns terracotta if it leaves the band. Arrow keys +
              Space/Enter work too. On Calories, hover the pace bar for the on-pace math.
            </>
          }
          asides={[
            {
              ...
              content: (
                <>
                  Weekly Plan: long-press-drag instead of mouse drag. A haptic tick on drop is
                  proposed, not built.
                </>
              ),
            },
        ...
        <GallerySection
          ...
          interaction={
            <>
              Hover a Build Phase milestone row for its projection math (assumptions + last test).
              Recent Sessions is static on web in this release — no tap target.
            </>
          }
          asides={[
            {
              ...
              content: (
                <>
                  Recent Sessions: swipe a row left → Edit only (72px, amber). Deletion is
                  intentionally absent from Home; it lives in session detail.
                </>
              ),
            },
          ]}
        ...
        <GallerySection
          ...
          interaction={
            <>
              Click the ‹ › arrows to page the heatmap's four-month window back through the block.
              Move across the VO2 trend for per-month values. The second VO2 card shows the empty
              state a widget collapses to with no Apple Health import — never a placeholder number.
            </>
          }
          asides={[
            {
              ...
              content: (
                <>
                  Both cards are desktop-only on Home (dropped below 720px, same breakpoint as the
                  sport analytics nav) — this section will disappear at phone widths here too. Widen
