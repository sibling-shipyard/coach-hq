...
describe("proactive Coach seed", () => {
  it("parses one valid seed and rejects absent, malformed, or repeated values", () => {
    expect(parseProactiveSeed(`?seed=${encodeURIComponent(SEED_ID)}`)).toBe(
      SEED_ID,
    );
    expect(parseProactiveSeed("")).toBeNull();
    expect(
      parseProactiveSeed("?seed=local-proactive-not-a-message"),
    ).toBeNull();
  ...
  it("reuses cached messages for the same seed without adding another opener", () => {
    ...
    expect(thread.messages.filter(({ role }) => role === "coach")).toHaveLength(
      1,
    );
  });
...
describe("sendMessage", () => {
  ...
  it("sends the explicit close flag and returns fresh profile completeness", async () => {
    ...
    expect(new Headers(request.headers).get("x-operation-id")).toMatch(
      /^[0-9a-f-]{36}$/,
    );
  ...
  it("does not retry an ambiguous POST network failure", async () => {
    ...
    await expect(sendMessage("thread-1", [], "hello", false)).rejects.toThrow(
      "network down",
    );
    expect(fetchMock).toHaveBeenCalledTimes(1);
  });

  it("does not retry a non-transient API response", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
...
describe("activitySync", () => {
  ...
  it("POSTs action activity_sync with the given activity_ids", async () => {
    ...
    const result = await activitySync([
    ...
    expect(
      coach && coach.role === "coach" ? coach.attachments : undefined,
...
describe("attachments on a thread", () => {
  ...
  it("keeps unknown attachment kinds on the thread but ignores them in the list helper", () => {
    ...
    expect(
      coach && coach.role === "coach" ? coach.attachments : undefined,
    ).toEqual([unknown, wrongVersion, listAttachment]);
    expect(syncedActivityList([unknown, wrongVersion])).toBeNull();
    expect(syncedActivityList([unknown, listAttachment])).toEqual(
      listAttachment,
    );
  });
  ...
  it("does not offer Retry once the Coach reply is on the turn", () => {
    expect(
      retryActivityIdsFromThread(threadWithAttachments([listAttachment])),
    ).toBeNull();
  });
...
describe("explicit close pending state", () => {
  it("keeps follow-up replies in closing mode for only the requested thread", () => {
    const pending = updatePendingEndThreads(
      new Set<string>(),
      "thread-1",
      true,
    );
  ...
  it("clears only after the requested thread closes", () => {
    ...
    expect(updatePendingEndThreads(pending, "thread-1", false)).toEqual(
      new Set(["thread-2"]),
    );
