...
- **Report shape, fixed.** Every worker report comes back as: files touched · checks run, **with
  ...
  The fourth field is not politeness. A brief once said to capture errors in
  `withContinuedTrace`'s catch, which sees nothing when a route answers with a status instead of
  throwing (`ui/api/coach-chat.ts`) — the worker saying so was worth more than its diff. Workers
  volunteer that unevenly; asking makes it reliable.
...
5. doc upkeep done (`AGENTS.md` § Doc upkeep) — including `SOUL_HISTORY` shape if soul changed.
	If the PR closes an issue, `grep -rn "#<N>" docs/ kdb/` first: a doc that still cites it for a
	gap the PR only half-closes will point readers at a closed issue
