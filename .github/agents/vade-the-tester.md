...
4. **Simulation suite** (`run-simulation-suite.ts`, paid, live model, real writes) — a tracked library of FSP/daily `--turns` scenarios run through kind 3's real pipeline, scored against an `expect` block, indexed in `tests/coverage-index.json`.
...
4. Append a run section to today's day-doc (format TBD until PR3 of the plan lands), update the coverage index.
...
- The day-doc style guide lands once PR3 of `docs/plans/vade-the-tester.md` merges — read it once it exists.
