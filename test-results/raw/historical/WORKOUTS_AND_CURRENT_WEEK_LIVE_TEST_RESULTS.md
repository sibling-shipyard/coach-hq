...
## Round 3: hardening the two remaining model-reliability gaps (PR #999, `bob/727-benchmark-never-fails`)

**Date:** 2026-09-13. Built on top of #999's own first-session-benchmark repair work (a different session's PR, independently reviewed and verified by me earlier - see PR review notes). Implements the four fixes I recommended against the two gaps above, after pushback on the two ideas I rejected (a discriminated `oneOf` schema - not achievable, Gemini's structured-output mode has no `oneOf` support, confirmed against `llmClient.ts` and `coachReplySchema.ts`'s own comment on the same limitation; and a prose-only-workout heuristic - real false-positive risk against ordinary conversation, skipped).

**Built:**
1. `findMalformedWorkoutCreateExercise` in `coachTurn.ts` - a `"reps"` exercise with no `reps` field, or `"timed"` with no `duration_secs`, now triggers the same one-shot corrective reprompt the pipeline already uses for over-length text fields.
2. A prompt-text sentence telling the model never to describe a new workout in reply text alone without also calling `workout_create`.
3. `newSessionMayDuplicatePlan` in `validateActions.ts` - a genuinely new deterministic guard (not a reuse of `categoryChangeIsConfirmed`, which only fires when an *existing* `session_id` is referenced - a brand-new session with no `session_id` previously got zero scrutiny). Deliberately narrow: only drops a brand-new session when it already claims a terminal status (`done`/`skipped`) on a date that still has a real `planned` session sitting there, and the athlete's message this turn doesn't confirm a genuine same-day extra. A day legitimately holding two sessions (a planned anchor plus a real unplanned extra) stays untouched.
4. A prompt-text sentence telling the model to check context for an existing session on that date first, before writing a brand-new done/skipped entry.

**Unit tests:** 8 new tests (3 for the reprompt in `coachTurn-reprompt.test.ts`, 5 for the duplicate-plan guard in `validateActions.test.ts`, including one confirming a genuine same-day extra is never blocked and one confirming the new 6th parameter defaults safely for every existing caller). Full `api/coach-chat/` suite: 557/557 pass, zero regressions. Full local gate (`bash platform/scripts/check.sh --quiet`): all 9 checks pass.

**Live re-test (5 real OpenRouter conversations, scratch branches off `coach-skanda-2003`, never touching `main`):**

- **Workout create** (`test/727-hardening-workout-create`): asked for a 4-exercise upper-body routine mixing reps and a timed plank hold. Committed clean on the first try - every `"reps"` exercise had a real `reps` value, the `"timed"` plank had a real `duration_secs`. No reprompt needed; confirms the new check doesn't fire on well-formed output (no false positive) and the workout itself came through correctly formed.
- **Same-day session completion, 3 variants** (`test/727-hardening-dup-session`, `-2`, `-3`): seeded a real `current_week.json` with a genuine `planned` "Strength A" session on today's date, then sent three different phrasings of "I did today's session, log it done" (direct, a claimed discipline swap to "mobility and recovery", and a terse "just wrapped up a session"). **All 5 runs across the 3 branches correctly referenced the real session's `session_id`** rather than inventing a duplicate - one run correctly triggered the *existing* `categoryChangeIsConfirmed` guard instead (the model tried to change the session's discipline without confirmation, which is a different, already-covered case). None of the 5 runs reproduced the original duplicate-session bug live - expected, since the original finding was intermittent (2 of 5 runs under the old prompt) and the new prompt reinforcement (fix 4 above) further lowers the odds of hitting it. The guard's actual drop behavior is verified deterministically by the unit tests instead, since forcing a live model into a specific mistake isn't reliably reproducible on demand.
- One test-setup bug surfaced along the way, not a product bug: my hand-seeded `current_week.json` fixture initially carried two stale fields (`coach_comments`, `planned_load`) that predate schema v1 and are no longer part of the real schema - `assertCurrentWeekCommitReady` correctly refused the patch result until the seed itself was fixed to match the real schema. Confirms the validator is doing its job.

**Status:** all four fixes pushed to `bob/727-benchmark-never-fails` (PR #999), commit `3cd7cf6e`, full gate green, CI pending at push time.
